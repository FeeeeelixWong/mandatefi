...
import {
  activationFundingRequirement, activationWalletReady, buildPortfolioPlan, formatNative, formatStable, GAS_LOW_WATERMARK, GAS_RESERVE, goalOptions,
...
function ActivityNarrative({ text }: { text: string }) {
  const summary = summarizeActivityText(text)
  const hasDetails = hasActivityDetails(text)

  return <>
    <p className="decision-summary">{summary}</p>
    {hasDetails && <details className="decision-technical-details">
...
function MandateWizard({
  ...
  const externalWalletReady = activationWalletReady(Boolean(account), isTargetNetwork, fundingRequirement.portfolioFunded)
  const ready = Boolean(externalWalletReady && altanaAddress && fundingReady && draft.stablecoinSelection)
      ...
      <section className="configurator-card">
        ...
        {step === 3 && <>
          ...
          <div className="approval-checklist">
            <div className={externalWalletReady ? 'complete' : ''}><span>{externalWalletReady ? <Check size={16} /> : <Wallet size={16} />}</span><div><strong>Funding wallet</strong><small>{fundingRequirement.portfolioFunded ? 'Not required · the Passkey wallet already holds managed capital' : account ? `${walletName} · ${shortAddress(account)} · ${!isTargetNetwork ? 'wrong network' : walletBalanceStatus === 'loading' ? 'reading balance' : walletBalanceStatus === 'error' ? 'balance unavailable' : `${walletBalance ?? '0'} tBNB available`}` : 'Choose an installed wallet for the first deposit'}</small></div>{!fundingRequirement.portfolioFunded && (!account ? <button onClick={onConnect}>Connect</button> : !isTargetNetwork ? <button onClick={onSwitchNetwork}>Switch</button> : null)}</div>
    ...
    {!hasActivity ? <div className="simple-empty"><Activity size={26} /><h2>No activity yet</h2><p>Create a strategy to generate its first decision.</p></div> : <div className="decision-log">
      ...
      {moduleReceipts.map(({ mandate, moduleReceipt }) => {
        ...
        return <article key={moduleReceipt.id}>
          ...
          <div className="decision-copy"><div><strong>{moduleReceipt.module} · {moduleReceipt.operation.replaceAll('_', ' ')}</strong><span>{mandate.name}</span></div><ActivityNarrative text={moduleReceipt.note} /><div className="decision-tags"><span>{shortAddress(moduleReceipt.contract)}</span>{moduleReceipt.inputAmount && <span>{moduleReceipt.inputAmount} {moduleReceipt.inputAsset}</span>}{moduleReceipt.outputAmount && <span>Received {moduleReceipt.outputAmount} {moduleReceipt.outputAsset}</span>}</div></div>
...
function App() {
  ...
  const portfolioAddress = activeMandate?.smartWallet ?? altana.address ?? null
  ...
  async function resumeMandateDeployment(id: string) {
    const mandate = mandates.find((item) => item.id === id)
    if (!mandate || resumingId) return
    if (mandate.status !== 'Active') {
      setNotice('Only an active strategy can resume deployment.')
      return
    }
    const remainingDays = Math.ceil((mandate.expiry * 1_000 - Date.now()) / (24 * 60 * 60 * 1_000))
    if (remainingDays <= 0) {
      setNotice('This strategy has expired. Its old permissions cannot be extended; create a new mandate only if you want to continue managing the portfolio.')
      return
    }
    ...
    setActivationAttempts((current) => [{
      id: attemptId,
      name: `Resume · ${mandate.name}`,
      createdAt: now,
      updatedAt: now,
      stablecoin: mandate.stablecoin,
      smartWallet: mandate.smartWallet,
      fundingAmount: mandate.managedAmount,
      phase: 'PREPARING',
      recoveryDetected: true,
      moduleReceipts: mandate.moduleReceipts,
    }, ...current])
    ...
    try {
      ...
      const completedModules = completedPancakeModules(mandate.moduleReceipts)
      updateAttempt({ phase: 'REVIEWING', moduleReceipts: mandate.moduleReceipts })
      const strategy = planForMandate(mandate)
      const executionPlan = buildPortfolioPlan({
        snapshot: latestSnapshot,
        managedAmount: safeParseStable(mandate.managedStableCap),
      ...
      try {
        ...
        // Missing execution evidence makes the deterministic activation gate fail closed.
      }
      ...
      if (!authorization.authorized) {
        throw new Error(`The existing strategy was recovered, but its missing modules remain fail-closed: ${authorization.blockers.join(' ')}`)
      }
      ...
      const mergedReceipts = [...(mandate.moduleReceipts ?? []), ...proof.receipts]
      const decision = buildDeploymentDecision(executionPlan, proof, review, true)
      runtimeSessions.current.set(id, proof.session)
      setRuntimeMandateIds((current) => current.includes(id) ? current : [...current, id])
      setPancakePositions(proof.positions)
